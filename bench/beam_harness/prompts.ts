// Exact BEAM prompts — copied verbatim from .research/BEAM/src/prompts.py
// Do not edit; harness hash pins this file.

export const answer_generation_for_rag = `You are an assistant that MUST answer questions using ONLY the information provided in the context below.
...
export const unified_llm_judge_base_prompt = `You are an expert evaluator tasked with judging whether the LLM's response demonstrates compliance with the specified RUBRIC CRITERION.
...
function replaceLiteral(template: string, token: string, value: string) {
  return template.replace(token, () => value);
}

export function buildAnswerPrompt(context: string, question: string) {
  return replaceLiteral(
    replaceLiteral(answer_generation_for_rag, "<context>", context || "No relevant memories found."),
    "<question>",
    question,
  );
}

export function buildJudgePrompt(question: string, rubricItem: string, llmResponse: string) {
  return replaceLiteral(
    replaceLiteral(
      replaceLiteral(unified_llm_judge_base_prompt, "<question>", question),
      "<rubric_item>",
      rubricItem,
    ),
    "<llm_response>",
...
export function buildOfficialJudgePrompt(rubricItem: string, llmResponse: string) {
  return replaceLiteral(
    replaceLiteral(unified_llm_judge_base_prompt, "<rubric_item>", rubricItem),
    "<llm_response>",
    llmResponse,
  );
}
