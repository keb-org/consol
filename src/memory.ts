...
export async function remember(vault: string, agentRoot: string, agent: string, input: RememberInput, db?: Database) {
  ...
  if (containsSecret({ statement: input.statement, scope: input.scope, refs })) throw new Error("secret rejected");
  if (refs.some((ref) => ref.length > 500 || /[\r\n\[\]]/.test(ref))) throw new Error("invalid ref");
...
function validateCaseData(data: Record<string, unknown>) {
  if (!stringField(data, "rootSource")) throw new Error("case rootSource required");
  if (!stringField(data, "task")) throw new Error("case task required");
  if (!stringField(data, "environment")) throw new Error("case environment required");
  if (!stringField(data, "action")) throw new Error("case action required");
  if (!stringField(data, "observableOutcome")) throw new Error("case observableOutcome required");
  if (!stringField(data, "evaluator")) throw new Error("case evaluator required");
  const outcome = stringField(data, "outcome");
  if (!outcome || !OUTCOME_VALUES.has(outcome)) throw new Error("case outcome must be success, failure, partial, or unknown");
  return stringRefs(data.appliedRefs, "appliedRefs");
}

function validateOutcomeData(data: Record<string, unknown>, refs: string[]) {
  const outcome = stringField(data, "outcome");
  if (!outcome || !OUTCOME_VALUES.has(outcome)) throw new Error("outcome must be success, failure, partial, or unknown");
  const evaluator = stringField(data, "evaluator");
  if (!evaluator || !EVALUATOR_VALUES.has(evaluator)) throw new Error("evaluator must be pass, fail, mixed, or unknown");
  const appliedRefs = stringRefs(data.appliedRefs, "appliedRefs");
  if (appliedRefs.some((ref) => !refs.includes(ref))) throw new Error("appliedRefs must also appear in refs");
  return appliedRefs;
}

export async function record(vault: string, agentRoot: string, agent: string, input: RecordInput) {
  if (!RECORD_KINDS.has(input.kind)) throw new Error(`unsupported record kind: ${input.kind}`);
  if (!input.data || typeof input.data !== "object" || Array.isArray(input.data)) throw new Error("data object required");
  const refs = [...new Set((input.refs ?? []).map((ref) => ref.trim()).filter(Boolean))];
  if (containsSecret({ data: input.data, refs })) throw new Error("secret rejected");
  if (refs.some((ref) => ref.length > 500 || /[\r\n]/.test(ref))) throw new Error("invalid ref");
...
export async function recordConsultedUsage(
  ...
  await withVaultLock(vault, async () => appendJsonl(usagePath(agentRoot), record));
  return record;
}
...
async function derivativeMutations(
  agentRoot: string,
  target: string,
  candidates: string[],
...
export async function forgetConfirm(vault: string, agentRoot: string, agent: string, target: string, confirmation: string, db?: Database) {
  ...
  const targetMatchesPlan = plan.targetHash
    ...
    : plan.target === target;
  if (!targetMatchesPlan) throw new Error("target mismatch");
  ...
  for (const candidate of candidates) {
    ...
    if (
      ...
      throw new Error("forget candidate escapes agent root or canonical note roots");
    }
  }
